...
class KeypointRCNNPredictorMulti(nn.Module):
    def __init__(self, cfg, in_channels):
        super(KeypointRCNNPredictorMulti, self).__init__()
        input_features = in_channels
        num_keypoints = cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_CLASSES
        deconv_kernel = 4
        self.kps_score_lowres = layers.ConvTranspose2d(
...
def make_roi_keypoint_predictor_multi(cfg, in_channels):
    return KeypointRCNNPredictorMulti(cfg, in_channels)
